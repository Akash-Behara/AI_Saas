...
const DashboardPage = () => {
  ...
  const tools = [
    {label: 'Conversations', icon: MessageSquare, color: 'text-violet-500', bgColor: 'bg-violet-500/10', href: '/conversation'},
  ...
  return (
    <div>
      <div className='mb-10 space-y-4'>
        ...
        <p className='text-muted-foreground text-center font-light text-sm md:text-lg'>Chat with the smartest AI - Experience the power of AI</p>
      </div>
      <div className='px-4 md:px-20 lg:px-32 space-y-4'>
        {tools.map((tool) => (
          <Card onClick={() => {router.push(tool.href)}} key={tool.label} className='p-4 border-black/5 flex items-center justify-between hover:shadow-md transition cursor-pointer'>
